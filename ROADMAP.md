...
The current development tree expands that proven three-direction baseline to two host visuals and nine format-2 WGSL presets. Cascading Falls reads shared spectrum history through a host-owned GPU storage buffer for its mirrored SDR-style trace and scrolling false-color waterfall. It adds bounded sound zones, camera interaction, per-band color/material routing, 10–35 grouped sliders per preset, named host-owned `.tvscene` snapshots, the 35-control procedural TheVisualCityScape flagship, a visible per-mode Instrument panel, and a local Visual Director brief composer. All nine presets loaded in the live development player on the NVIDIA adapter, and Cascading Falls visibly advanced live spectrum history downward. Named scene save, restart discovery, and cross-mode restore passed a live isolated-directory check; the previous eight-preset portable package snapshot passed its build, staged/fresh checksum, discovery, live-capture, preset-rendering, onset-telemetry, and default-disabled-plugin smoke on the development host. Cascading Falls and the later Feedback Tunnel timing/material and Solar Bloom material refinements remain development-tree-only pending a future approved package cycle. Clean-machine package validation remains open. Paid image generation is not connected.
