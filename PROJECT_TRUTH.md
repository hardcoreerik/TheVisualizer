...
- The `Living Photograph` image layer uses one bounded 24×14 host-painted image mesh. Mid energy produces anchored foliage motion, low energy produces subpixel table motion and slow cloud exposure, and RMS produces a localized breathing motion around the frog without adding a second renderer or image pipeline.
- Thirty-six bounded photo-space rain streaks and seven bounded insects respond to treble energy. A muted bird silhouette crosses the greenhouse glass for 2.4 seconds on a deterministic 53-second interval, with wing motion accelerated by treble and visibility influenced by transient strength.
...
- The package script stages the bundled Studio image and its provenance record.
