...
- `P` opens a persistent Performance Studio with two keyed GPU-preset renderer instances, a full-frame A/B crossfader, ten bounded transition curves, ten macro controls, selectable Full/Bass/Mid/Treble/Transient/Onset modulation, attack/release/curve/inversion, per-deck level/speed/hue/freeze/mute, deck swap, output freeze, and blackout. Host-rendered modes can be assigned and taken directly, but only WGSL preset decks participate in the current simultaneous live mix; spatial transition masks, video/webcam layers, controller input, and external video output are not yet implemented.
- An experimental Studio visual composites a bounded stack of up to eight Image, GPU Preset, Waveform, and Particles layers. It permits one opaque GPU-preset base, one local image, and repeated host-painted waveform or particle layers. Layers expose visibility, ordering where supported, opacity where supported, audio-band routing, reactivity, scale, and Normal/Add blending where supported.
