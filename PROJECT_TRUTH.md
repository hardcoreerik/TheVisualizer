...
- Particle Forge renders a sparse local copy of the forge at each sound zone and exposes direction reversal plus continuous gradient shifting in Mode Controls.
...
- Thirty-seven focused player tests cover analysis and onset hysteresis, repeated decaying attacks, quiet-noise rejection, deterministic cadence stability, resettable onset telemetry, callback-sequence gating, quiet-state guidance, frame pacing, direct visual shortcuts, devices, same-ID capture-failure recovery, timestamp adjustment, latency aggregation, frame-cadence smoothing, default-switch timing, bounded visual history and Studio layer stack, bundled still and 49-frame motion decoding, seamless ping-pong indexing, foliage-rig segmentation, interaction/color bounds and band cycling, Particle Forge gradient wrapping, presentation, presets, strict saved-scene round-trip and bounds, Visual Director rolling history, structured novelty, durable brief-history reload, capture/output planning, the WGSL scalar/feature/scene/parameter/history buffer contract, and strict plugin manifests. Separate SDK and example-plugin lifecycle tests also pass.
